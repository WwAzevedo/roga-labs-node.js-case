...
const peopleModel = {
  ...
  createPerson: async (nome, nome_mae, nome_pai, cep, data_nascimento, data_cadastro, data_edicao) => {
    try {
  ...
  updatePerson: async (id, nome, nome_mae, nome_pai, cep, data_nascimento, data_edicao) => {
    try {
      const { rows } = await pool.query(
